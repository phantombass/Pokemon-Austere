...
class PokemonPartyConfirmCancelSprite < SpriteWrapper
  ...
  def initialize(text,x,y,narrowbox=false,viewport=nil)
    ...
    @yoffset = 8
    textpos = [[text,16,-8,0,Color.new(0,0,0),Color.new(255,255,255,0)]]
...
class PokemonPartyCancelSprite < PokemonPartyConfirmCancelSprite
  def initialize(viewport=nil)
    super(_INTL("CANCEL"),0,208,false,viewport)
  end
end
...
class PokemonPartyConfirmSprite < PokemonPartyConfirmCancelSprite
  def initialize(viewport=nil)
    super(_INTL("CONFIRM"),0,208,true,viewport)
  end
end
...
class PokemonPartyCancelSprite2 < PokemonPartyConfirmCancelSprite
  def initialize(viewport=nil)
    super(_INTL("CANCEL"),160,208,true,viewport)
  end
end
...
class PokemonPartyBlankPanel < SpriteWrapper
  ...
  def initialize(_pokemon,index,viewport=nil)
    super(viewport)
    self.x = [0,0,0,0,0,0][index]
    self.y = [0,32,64,96,128,160][index]
...
class PokemonPartyPanel < SpriteWrapper
  ...
  def refresh
    ...
    if @pkmnsprite && !@pkmnsprite.disposed?
      @pkmnsprite.x        = self.x+4
      @pkmnsprite.y        = self.y+6
      @pkmnsprite.color    = self.color
      @pkmnsprite.selected = self.selected
    end
    if @helditemsprite && !@helditemsprite.disposed?
      if @helditemsprite.visible
        @helditemsprite.x     = self.x
        @helditemsprite.y     = self.y+20
        @helditemsprite.color = self.color
      end
	  if self.selected
	    @helditemsprite.x     = self.x+16
	  end
    end
    if @overlaysprite && !@overlaysprite.disposed?
      @overlaysprite.x     = self.x
    ...
    if @refreshBitmap
      ...
      textpos = []
      # Draw Pokémon name
      textpos.push([@pokemon.name,48,8,0,basecolor,shadowcolor])
      if !@pokemon.egg?
        if !@text || @text.length==0
          # Draw HP numbers
          textpos.push([sprintf("% 3d/% 3d",@pokemon.hp,@pokemon.totalhp),320,8,1,basecolor,shadowcolor])
          # Draw HP bar
          if @pokemon.hp>0
            ...
            @overlaysprite.bitmap.blt(208,38,@hpbar.bitmap,hprect)
          end
          # Draw status
          status = -1
          ...
          if status>=0
            statusrect = Rect.new(0,14*status,46,14)
            @overlaysprite.bitmap.blt(64,32,@statuses.bitmap,statusrect)
          end
        end
        # Draw gender symbol
        if @pokemon.male?
          textpos.push([_INTL("♂"),48,25,0,Color.new(0,0,255)])
        elsif @pokemon.female?
          textpos.push([_INTL("♀"),48,25,0,Color.new(255,0,0)])
        end
        # Draw shiny icon
        if @pokemon.shiny? && status == -1
          pbDrawImagePositions(@overlaysprite.bitmap,[[
             "Graphics/Pictures/shiny",64,31,0,0,16,16]])
        end
      end
      pbDrawTextPositions(@overlaysprite.bitmap,textpos)
      # Draw level text
      if !@pokemon.egg? # If the Pokémon is an EGG, nothing of this will appear
	    if @pokemon.level < 100
          pbDrawImagePositions(@overlaysprite.bitmap,[[
            "Graphics/Pictures/Party/overlay_lv",128,32,0,0,22,14]])
	    end
	    if @pokemon.level < 10
          pbDrawTextPositions(@overlaysprite.bitmap,[
            [@pokemon.level.to_s,160,24,1,basecolor,shadowcolor]])
	    else
	      pbDrawTextPositions(@overlaysprite.bitmap,[
            [@pokemon.level.to_s,176,24,1,basecolor,shadowcolor]])
	    end
      end
      ...
      if @text && @text.length>0
        pbSetSystemFont(@overlaysprite.bitmap)
        pbDrawTextPositions(@overlaysprite.bitmap,[
           [@text,190,24,0,basecolor,shadowcolor]
        ])
      end
    end
    @refreshing = false
...
class PokemonPartyScreen
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    ...
    @scene.pbStartScene(@party,_INTL("Choose a POKéMON."),annot,true)
    loop do
      realorder = []
      for i in 0...@party.length
      ...
      command = @scene.pbShowCommands("",commands) if pkmn
  ...
  def pbChooseAblePokemon(ableProc,allowIneligible=false)
    ...
    @scene.pbStartScene(@party,
       (@party.length>1) ? _INTL("Choose a POKéMON.") : _INTL("Choose a POKéMON."),annot)
    loop do
      @scene.pbSetHelpText(
