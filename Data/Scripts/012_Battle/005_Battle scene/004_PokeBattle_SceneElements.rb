...
class PokemonDataBox < SpriteWrapper
  ...
  NAME_SHADOW_COLOR       = Color.new(255,255,255,0)
  MALE_BASE_COLOR         = Color.new(0,0,255)
  MALE_SHADOW_COLOR       = NAME_SHADOW_COLOR
  FEMALE_BASE_COLOR       = Color.new(255,0,0)
  ...
  def initializeDataBoxGraphic(sideSize)
    ...
    if onPlayerSide
      @spriteX = Graphics.width - 194 #244
    ...
    when 2
	if onPlayerSide
      @spriteX += [8,  -20,  8,  -20][@battler.index]
      @spriteY += [-16, 32, 32, 2][@battler.index]
	else
	  @spriteX += [-44,  -16,  -44,  -16][@battler.index]
      @spriteY += [0, 48, 48, 0][@battler.index]
	end
    when 3
  ...
  # HP Bar, Exp Bar and HP Numbers X values
  def x=(value)
    super
	if @sideSize != 1
      if @showHP # On PLayer's Side
	    @hpBar.x     = value+@spriteBaseX+26
	  else
	    @hpBar.x     = value+@spriteBaseX+30
	  end
	else
	  if @showHP # On Player's Side
        @hpBar.x     = value+@spriteBaseX+32
      else
        @hpBar.x     = value+@spriteBaseX+36
	  end
    end
  ...
  # HP Bar, Exp Bar and HP Numbers Y values
  def y=(value)
    super
    @hpBar.y     = value+38
    @expBar.y    = value+70
  ...
  def refreshHP
    ...
    if @showHP
      ...
      pbDrawNumber(-1,@hpNumbers.bitmap,69,2,1)   # / char
      pbDrawNumber(@battler.totalhp,@hpNumbers.bitmap,118,2,1)
    end
    # Resize HP bar
    w = 0
    if self.hp>0
...
class AbilitySplashBar < SpriteWrapper
  ...
  def initialize(side,viewport=nil)
    ...
    @contents = BitmapWrapper.new(@bgBitmap.width,@bgBitmap.height)
    self.bitmap = @contents
  ...
  def refresh
    ...
    textPos.push([(@ability.is_a?(String))? @ability : @battler.abilityName,textX,8,@side==1,
       TEXT_BASE_COLOR,TEXT_SHADOW_COLOR,true])
    pbDrawTextPositions(self.bitmap,textPos)
  end
