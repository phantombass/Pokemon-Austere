...
$VersionStyles = [
  ...
  #["Power Red and Green"],
  #["Power Clear"],
  ["Pokemon GSC font (European version rip)"]
]
...
class Window_PokemonOption < Window_DrawableCommand
  ...
  def initialize(options,x,y,width,height)
    @options = options
    @nameBaseColor   = Color.new(0,0,0)
    @nameShadowColor = Color.new(255,255,255,0)
    @selBaseColor    = Color.new(255,0,0)
    @selShadowColor  = Color.new(255,255,255,0)
    @optvalues = []
  ...
  def drawItem(index,_count,rect)
    ...
    if @options[index].is_a?(EnumOption)
      ...
      value = _INTL("{1}/{2}",@options[index].optstart+self[index],
