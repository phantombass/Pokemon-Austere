...
class Window_AdvancedTextPokemon < SpriteWindow_Base
  ...
  def allocPause
    ...
    @pausesprite = AnimatedSprite.create("Graphics/Pictures/pause",2,12)
...
module UpDownArrowMixin
  ...
  # Function for not showing arrows in some UIs
  def hideArrows(hide)
    @hideArrow = hide
  end
  ...
  def update
    ...
    @uparrow.update
    @downarrow.update
    @uparrow.visible = @downarrow.visible = !@hideArrow if @hideArrow
  end
end
