...
def pbStorePokemon(pokemon)
  ...
  if $Trainer.party.length<6
    ...
    if storedbox!=oldcurbox
      if creator
        pbMessage(_INTL("\'{1}\' on {2}'s PC was full.\1",curboxname,creator))
      else
        pbMessage(_INTL("\'{1}\' on someone's PC was full.\1",curboxname))
      end
      pbMessage(_INTL("{1} was transferred to \'{2}.\'",pokemon.name,boxname))
    else
      if creator
