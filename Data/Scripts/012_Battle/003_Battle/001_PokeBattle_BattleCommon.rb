module PokeBattle_BattleCommon
  ...
  def pbStorePokemon(pkmn)
    ...
    if storedBox!=currentBox
      if creator
        pbDisplayPaused(_INTL("\'{1}\' on {2}'s PC was full.",curBoxName,creator))
      else
        pbDisplayPaused(_INTL("\'{1}\' on someone's PC was full.",curBoxName))
      end
      pbDisplayPaused(_INTL("{1} was transferred to \'{2}\'.",pkmn.name,boxName))
    else
      if creator
  ...
  def pbRecordAndStoreCaughtPokemon
    @caughtPokemon.each do |pkmn|
      ...
      if !pbPlayer.hasOwned?(pkmn.species)
        ...
        if $Trainer.pokedex
          pbDisplayPaused(_INTL("{1}'s data was newly added to the POKéDEX.",pkmn.name))
