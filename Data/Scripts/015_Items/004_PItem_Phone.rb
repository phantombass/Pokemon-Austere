...
def pbPhoneRegisterNPC(ident,name,mapid,showmessage=true)
  ...
  pbMessage(_INTL("\\me[Register phone]{1} registered {2}'s number.\\wtnp[40]",$Trainer.name,name)) if showmessage
end
...
def pbPhoneRegisterBattle(message,event,trainertype,trainername,maxbattles)
  ...
  pbMessage(_INTL("\\me[Register phone]{1} registered {2}'s number.\\wtnp[40]",$Trainer.name,displayname))
end
...
def pbPhoneCall(call,phonenum)
  pbMessage(_INTL("\\se[Call]......\\wt[5] ......\\1"))
  ...
  pbMessage(_INTL("\\se[CallEnd]Click!\\wt[10]\n......\\wt[10] ......\\wt[10] ......\\1"))
end
