...
class CommandMenuDisplay < BattleMenuBase
  ...
  MODES = [
     ...
     [0,2,8,3]    # 4 = Bug Catching Contest
  ]
  ...
  def initialize(viewport,z)
    ...
    if USE_GRAPHICS
      ...
      @buttons = Array.new(4) do |i|   # 4 command options, therefore 4 buttons
        ...
        button.x      = self.x+Graphics.width-174
        button.x      += (((i%2)==0) ? 0 : @buttonBitmap.width/2-12) #-4
        button.y      = self.y+32 #6
        button.y      += (((i/2)==0) ? 0 : BUTTON_HEIGHT+16) #-4
...
class FightMenuDisplay < BattleMenuBase
  ...
  def initialize(viewport,z)
    ...
    if USE_GRAPHICS
      ...
      # Create background graphic
      background = IconSprite.new(0,128,viewport) # Graphics.height-96
      ...
      @buttons = Array.new(MAX_MOVES) do |i|
        ...
        button.bitmap = @buttonBitmap.bitmap
        button.x      = self.x+82
        #button.x      += (((i%2)==0) ? 0 : @buttonBitmap.width/2-0)
        button.y      = self.y+80 + 16*i
        #button.y      += (((i/2)==0) ? 0 : BUTTON_HEIGHT_FIGHT+16)
        button.src_rect.width  = @buttonBitmap.width/2
        button.src_rect.height = BUTTON_HEIGHT_FIGHT
        addSprite("button_#{i}",button)
        next button
      end
      ...
      @typeIcon.x      = self.x+32
      @typeIcon.y      = self.y+32
      ...
      @megaButton.x      = self.x+196
      @megaButton.y      = self.y-@megaEvoBitmap.height/2+62
      ...
      @msgBox = Window_AdvancedTextPokemon.newWithSize("",
         self.x+16,self.y+16,Graphics.width,Graphics.height-self.y,viewport)
      @msgBox.baseColor   = TEXT_BASE_COLOR
      @msgBox.shadowColor = TEXT_SHADOW_COLOR
      pbSetSystemFont(@msgBox.contents)
      addSprite("msgBox",@msgBox)
      # Create command window (shows moves)
      @cmdWindow = Window_CommandPokemon.newWithSize([],
         self.x,self.y,320,Graphics.height-self.y,viewport)
      @cmdWindow.columns       = 1
      @cmdWindow.columnSpacing = 2
      @cmdWindow.ignore_input  = true
  ...
  def refreshButtonNames
    ...
    moves.each_with_index do |m,i|
      button = @buttons[i]
	  textPos.push(["-",96,72+16*i,0,TEXT_BASE_COLOR,TEXT_SHADOW_COLOR]) if !@visibility["button_#{i}"]
      next if !@visibility["button_#{i}"]
      x = 96#button.x-self.x+button.src_rect.width/2-7
      y = 72+16*i#button.y-self.y-8
      ...
      if m.type>=0
        ...
        #moveNameBase = button.bitmap.get_pixel(10,button.src_rect.y+34) #10, y+34
      end
      textPos.push([m.name,x,y,0,moveNameBase,TEXT_SHADOW_COLOR])
    end
	blankmove = 4 - moves.length; i = moves.length
	blankmove.times do
		textPos.push(["-",96,72+16*i,0,moveNameBase,TEXT_SHADOW_COLOR])
		i+=1
	end
  ...
  def refreshSelection
    ...
    if USE_GRAPHICS
      ...
      @buttons.each_with_index do |button,i|
        ...
        button.src_rect.x = (i==@index) ? @buttonBitmap.width/2 : 0
		button.src_rect.y = moves[i].type*BUTTON_HEIGHT_FIGHT
  ...
  def refreshMoveData(move)
    ...
    # PP text
	textPos = [
		[_INTL("TYPE/"),16,8,0,TEXT_BASE_COLOR,TEXT_SHADOW_COLOR],
        [_INTL("{1}",PBTypes.getName(move.type)),84,24,2,TEXT_BASE_COLOR,TEXT_SHADOW_COLOR]
		]
    if move.totalpp>0
      ppFraction = [(4.0*move.pp/move.totalpp).ceil,3].min
      #textPos = []
      textPos.push([_INTL("{1}/{2}",move.pp,move.totalpp),
         160,40,1,PP_COLORS[ppFraction*2],PP_COLORS[ppFraction*2+1]])
...
class TargetMenuDisplay < BattleMenuBase
  ...
  CMD_BUTTON_WIDTH_SMALL = 160
  TEXT_BASE_COLOR   = Color.new(0,0,0)
  TEXT_SHADOW_COLOR = Color.new(255,255,255,0)
  ...
  def initialize(viewport,z,sideSizes)
    ...
    @buttons = Array.new(maxIndex+1) do |i|
      ...
      button.src_rect.height = 46
      if @smallButtons
        button.x    = self.x+116-[0,82,166][numButtons-1]
      else
        button.x    = self.x+116-[0,116][numButtons-1]
      end
      button.x      += (button.src_rect.width)*inc
      button.y      = self.y+4
      button.y      += (46)*((i+1)%2)
      addSprite("button_#{i}",button)
      next button
  ...
  def refreshButtons
    ...
    @buttons.each_with_index do |button,i|
      ...
      button.src_rect.y = buttonType*46
