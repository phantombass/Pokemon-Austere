...
class PokemonPauseMenu
  ...
  def pbStartPokemonMenu
    ...
    commands[cmdBag = commands.length]      = _INTL("PACK") if !pbInBugContest?
    commands[cmdPokegear = commands.length] = _INTL("~€GEAR") if $Trainer.pokegear
    ...
    if pbInSafari?
      ...
      commands[cmdQuit = commands.length]   = _INTL("EXIT")
    ...
    commands[cmdEndGame = commands.length]  = _INTL("QUIT") if $DEBUG
    loop do
      ...
      if cmdPokedex>=0 && command==cmdPokedex
        ...
        pbFadeOutIn {
          scene = TrainerCard.new.cardFront
          screen = TrainerCard.new
          @scene.pbRefresh
        }
