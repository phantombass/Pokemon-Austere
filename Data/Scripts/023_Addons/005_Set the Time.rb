...
module SetTime
  ...
  def set(time)
    ...
    PBDayNight.schedule_tone_refresh
  end
...
module PBDayNight
  class << self
    ...
    # schedule a tone refresh on the next try (probably next frame)
    def schedule_tone_refresh
...
# Work in new and old ruby when checking top-level methods
def m_defined?(sym)
  return (
    Object.private_method_defined?(sym) || Object.public_method_defined?(sym)
  )
end
