...
class BattleIntroAnimation < PokeBattle_Animation
  ...
  def createProcesses
    appearTime = 30   # This is in 1/20 seconds
    # Background
...
class BattleIntroAnimation2 < PokeBattle_Animation
  ...
  def createProcesses
    for i in 0...@sideSize
      ...
      battler.moveTone(0,1,Tone.new(0,0,0,0))
...
#===============================================================================
# Shows Player backsprite fading back to their normal color, and triggers their intro
# animations
#===============================================================================
class BattleIntroAnimation3 < PokeBattle_Animation
  def initialize(sprites,viewport,battle)
    @battle = battle
    super(sprites,viewport)
  end
  ...
  def createProcesses
    @battle.player.each_with_index do |_p,i|
	  next if !@sprites["player_#{i+1}"]
	  trainer = addSprite(@sprites["player_#{i+1}"],PictureOrigin::Bottom)
      trainer.moveTone(0,1,Tone.new(0,0,0,0))
    end
  end
end
...
#===============================================================================
# Shows Trainer fading back to their normal color, and triggers their intro
# animations
#===============================================================================
class BattleIntroAnimation4 < PokeBattle_Animation
  def initialize(sprites,viewport,battle)
    @battle = battle
    super(sprites,viewport)
  end
...
class LineupAppearAnimation < PokeBattle_Animation
  ...
  def resetGraphics(sprites)
    ...
    when 0   # Player's lineup
      barX  = Graphics.width - BAR_DISPLAY_WIDTH + 72
      barY  = Graphics.height - 130
      ballX = barX + 34
      ballY = barY + 4
  ...
  def createProcesses
    ...
    for i in 0...PokeBattle_SceneConstants::NUM_BALLS
      ...
      createBall(i,(@fullAnim) ? delay : 0,dir)
    end
  end
...
class PlayerFadeAnimation < PokeBattle_Animation
  ...
  def createProcesses
    ...
    if @sprites["partyBar_0"] && @sprites["partyBar_0"].visible
      ...
      partyBar.setVisible(delay,false)
      partyBar.setOpacity(delay,255)
    end
    for i in 0...PokeBattle_SceneConstants::NUM_BALLS
      ...
      partyBall.setVisible(delay,false)
      partyBall.setOpacity(delay,255)
    end
  end
end
...
class PokeballPlayerSendOutAnimation < PokeBattle_Animation
  ...
  def createProcesses
    ...
    ball.setVisible(0,false) # invisible pokeball
    delay = ball.totalDuration   # 0 or 7
    # Poké Ball trajectory animation
    createBallTrajectory(ball,delay,12,
       ballStartX,ballStartY,ballMidX,ballMidY,battlerStartX,battlerStartY-18,false)
    ...
    # ball out anim
    smoke = addNewSprite(battlerStartX+80,battlerStartY-48,
    sprintf("Graphics/Battle animations/ball_out_animation"),PictureOrigin::Center)
    size=80
    smoke.setSrcSize(0,80,80)
    delay = ball.totalDuration
    smoke.setSrc(0,0,0)
    smoke.setVisible(0,false)
    smoke.setVisible(delay,true)
    smoke.setZ(0,batSprite.z+1)
    smoke.setSrc(delay+2,size,0)
    smoke.setSrc(delay+4,size*2,0)
    smoke.setVisible(delay+8,false)
...
class PokeballTrainerSendOutAnimation < PokeBattle_Animation
  ...
  def createProcesses
    ...
    ball.moveOpacity(delay+2,2,0)
     # ball out anim
     smoke = addNewSprite(battlerStartX+80,battlerStartY-40,
     sprintf("Graphics/Battle animations/ball_out_animation"),PictureOrigin::Center)
     size=80
     smoke.setSrcSize(0,80,80)
     delay = ball.totalDuration
...
class PokeballThrowCaptureAnimation < PokeBattle_Animation
  ...
  def createProcesses
    ...
    # ball out anim
    smoke = addNewSprite(ballEndX+80,ballEndY,
    sprintf("Graphics/Battle animations/ball_out_animation"),PictureOrigin::Center)
    size=80
    smoke.setSrcSize(0,80,80)
    delay = ball.totalDuration
