class PokeBattle_Scene
  ...
  def pbInitSprites
    ...
    for side in 0...2
      partyBar = pbAddSprite("partyBar_#{side}",0,0,
         "Graphics/Pictures/Battle/overlay_teampreview",@viewport)
      partyBar.z       = 120
      partyBar.mirror  = true if side==1   # Rival's lineup bar only
    ...
    if @battle.wildBattle?
      @battle.pbParty(1).each_with_index do |pkmn,i|
        ...
        pkmnSprite.tone    = Tone.new(-60,-60,-60,255)
  ...
  def pbCreateTrainerBackSprite(idxTrainer,trainerType,numTrainers=1)
	...
	trainer.tone = Tone.new(-60,-60,-60,255)
  end
