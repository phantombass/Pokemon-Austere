...
class PokemonBoxArrow < SpriteWrapper
  ...
  def y=(value)
    ...
    heldPokemon.y = self.y-8 if holding? #+16
  end
...
class PokemonBoxSprite < SpriteWrapper
  ...
  def initialize(storage,boxnumber,viewport=nil)
    ...
    @contents = BitmapWrapper.new(320,288)
  ...
  def refresh
    if @refreshBox
      ...
      getBoxBitmap
      @contents.blt(0,0,@boxbitmap.bitmap,Rect.new(0,0,198,198))
      pbSetSystemFont(@contents)
      widthval = @contents.text_size(boxname).width
      xval = 100-(widthval/2)
      pbDrawShadowText(@contents,xval,-11,widthval,32,boxname,Color.new(0,0,0),Color.new(255,255,255,0))
      @refreshBox = false
    end
    yval = self.y+32 #30
    for j in 0...5
...
class PokemonBoxPartySprite < SpriteWrapper
  def initialize(party,viewport=nil)
    ...
    @contents = BitmapWrapper.new(128,192)
    self.bitmap = @contents
    self.x = 0 #182
    self.y = Graphics.height-288
  ...
  def refresh
    ...
    xvalues = [25,79,25,79,25,79]
    yvalues = [22,22,68,68,114,114]
    for j in 0...6
...
class PokemonStorageScene
  ...
  def pbShowCommands(message,commands,index=0)
    ...
    msgwindow.letterbyletter = false
    msgwindow.text           = message
    msgwindow.resizeHeightToFit(message,Graphics.width-0)
    pbBottomRight(msgwindow)
  ...
  def pbSetArrow(arrow,selection)
    ...
    when -3 # Close Box
      ...
      arrow.x = (0+33*(selection%6))#*2
      arrow.y = (44+32*(selection/6))#*2
    end
  end
  ...
  def pbPartySetArrow(arrow,selection)
    if selection>=0
      xvalues = [5,32,5,32,5,32,5]
      yvalues = [16,16,40,40,64,64,80]
      arrow.angle = 0
      arrow.mirror = false
      arrow.ox = 0
      arrow.oy = 0
  ...
  def pbChangeBackground(wp)
    ...
    timeTaken.times do
      ...
      Graphics.update
      Input.update
      @sprites["box"].color = Color.new(0,0,0,alpha)
      self.update
    end
  ...
  def pbSwitchBoxToRight(newbox)
    ...
    loop do
      Graphics.update
      Input.update
      @sprites["box"].x -= 16 #distancePerFrame
      newbox.x -= 0 #distancePerFrame
      self.update
      break if newbox.x<=0
    end
    diff = newbox.x-0
  ...
  def pbShowPartyTab
    ...
    loop do
      Graphics.update
      Input.update
      @sprites["boxparty"].y -= 16 #distancePerFrame
      self.update
      break if @sprites["boxparty"].y<=Graphics.height-288
    end
    @sprites["boxparty"].y = Graphics.height-288
  end
  ...
  def pbUpdateOverlay(selection,party=nil)
    ...
    overlay.clear
    buttonbase = Color.new(0,0,0)
    buttonshadow = Color.new(255,255,255,0)
    ...
    base   = Color.new(0,0,0)
    shadow = Color.new(255,255,255,0)
    nonbase   = Color.new(0,0,0)
    nonshadow = Color.new(255,255,255,0)
    pokename = pokemon.name
    textstrings = [
       [pokename,2,200,false,base,shadow]
    ]
    if !pokemon.egg?
      imagepos = []
      if pokemon.male?
        textstrings.push([_INTL("♂"),288,140,false,Color.new(0,0,255),shadow])
      elsif pokemon.female?
        textstrings.push([_INTL("♀"),288,140,false,Color.new(255,0,0),shadow])
      end
	  if pokemon.level !=100
      imagepos.push(["Graphics/Pictures/Storage/overlay_lv",222,152])
	  end
      textstrings.push([pokemon.level.to_s,237,140,false,base,shadow])
      if pokemon.ability>0
        textstrings.push([_INTL("AB./ {1}",PBAbilities.getName(pokemon.ability)),2,224,0,base,shadow])
      else
        textstrings.push([_INTL("NO ABILITY"),2,224,0,nonbase,nonshadow])
      end
      if pokemon.item>0
        textstrings.push([PBItems.getName(pokemon.item),2,248,0,base,shadow])
      else
        textstrings.push([_INTL("NO ITEM"),2,248,0,nonbase,nonshadow])
      end
      if pokemon.shiny?
        imagepos.push(["Graphics/Pictures/shiny",304,140])
      end
      typebitmap = AnimatedBitmap.new(_INTL("Graphics/Pictures/Storage/types"))
      type1rect = Rect.new(0,pokemon.type1*14,110,14)
      type2rect = Rect.new(0,pokemon.type2*14,110,14)
      if pokemon.type1==pokemon.type2
        overlay.blt(206,164,typebitmap.bitmap,type1rect)
      else
        overlay.blt(206,164,typebitmap.bitmap,type1rect)
        overlay.blt(206,180,typebitmap.bitmap,type2rect)
      end
      drawMarkings(overlay,215,5,128,20,pokemon.markings)
...
### DEPOSIT ####################################################################
      ...
      loop do
        ...
        if selected==-3   # Close box
          ...
          when 2; pbRelease([-1,selected],nil)
          end
        end
      end
      @scene.pbCloseBox
