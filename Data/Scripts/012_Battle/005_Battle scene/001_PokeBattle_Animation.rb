...
module PokeBattle_BallAnimationMixin
  ...
  def addBallSprite(ballX,ballY,ballType)
    ...
    if @ballSprite.bitmap.width>=@ballSprite.bitmap.height
      ...
      ball.setSrcSize(0,@ballSprite.bitmap.height,@ballSprite.bitmap.height) #height/2
    end
    return ball
  end
  ...
  def createBallTrajectory(ball,delay,duration,startX,startY,midX,midY,endX,endY,visible=true)
...
=end
  def ballSetOpen(ball,delay,ballType)
    ...
    if @ballSprite && @ballSprite.bitmap.width>=@ballSprite.bitmap.height
      ball.setSrcSize(delay,@ballSprite.bitmap.height,@ballSprite.bitmap.height) #height/2
    end
  end
  ...
  def battlerAppear(battler,delay,battlerX,battlerY,batSprite,color)
    battler.setVisible(delay+6,true)
